...
class VectorDatabase(Config):
    ...
    def create_passages(self, documents: list) -> list:
        """
    ...
    def create_small_passages(self, documents: list) -> list:
        """
        Splits the documents into passages of a certain length
        Args:
            documents (list): list of documents
        Returns:
            list: list of passages
        """
        return [item for row in [self.text_splitter.split_documents(d) for d in documents] for item in row]
